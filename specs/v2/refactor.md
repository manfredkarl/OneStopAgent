...
| **During execution** | Any message while an agent is running | Queue the message. Do NOT interrupt the running agent. After the current agent completes, PM processes the queued message before advancing to the next step. Show: "I'll address that after the current step completes." |
...
* **Primary region**: Use the first mentioned region for all services in `state.services.selections`
* **HA/DR note**: Add a note to each service: "For high availability, deploy a secondary instance in {second_region}"
* Do NOT duplicate every service into two entries — that inflates the cost estimate unrealistically
* Instead, add ONE line item: "Multi-region replication overhead" with an estimated 30-50% uplift on compute + storage costs
* The Architect Agent should reflect multi-region in the Mermaid diagram (show both regions as deployment targets)
...
The Anthropic PPTX skill works by providing Claude with a structured prompt containing all slide content, and Claude generates the `.pptx` file directly as a file artifact. To integrate:

1. Call Azure OpenAI (or Anthropic API if available) with the slide content structured per the skill's prompt template
2. If the model supports file artifact generation → receive the `.pptx` binary directly
3. If the model does NOT support file artifacts (e.g., Azure OpenAI GPT-4.1) → the LLM generates slide content as structured JSON, and `python-pptx` renders it to `.pptx`

This means in practice for MVP: the LLM generates the slide CONTENT (titles, bullets, tables), and `python-pptx` renders the physical file. The Anthropic skill's prompt engineering approach is used for content quality, even if the file isn't generated by Claude directly.
